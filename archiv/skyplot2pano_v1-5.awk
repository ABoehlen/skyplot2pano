...
#               - die verwendeten Namensfiles muessen im gleichen Textformat vorliegen wie fuer die Benutzung mit SCOP.PER
#               - das verwendete Namensfile muss in dem Ordner liegen, wo die Berechnung vorgenommen wird
#               - das SCOP Utility Programm skyplot.exe muss vorhanden und über den Befehl 'skyplot' aufrufbar sein
#
# Usage:        skyplot2pano.awk  <X> <Y> <Z> <Name> <DHM> <Aufloes-Azi> <Azi links> <Azi rechts> <Bildbreite> <Min-Dist> <Max-Dist> <Aufloes-Dist> <Nam>
#
#########################################################################################################################################################
          ...
          if (existiertPkt == 0) {
            ...
            if (namFile != "0") {
              for (nam = 1; nam <= anzNam; nam++) {
                # Lagetoleranz in x/y je 50 m
                if ((xyPt[1] - namX[nam]) >= -50 && (xyPt[1] - namX[nam]) <= 50) {
                  if ((xyPt[2] - namY[nam]) >= -50 && (xyPt[2] - namY[nam]) <= 50) {
                    ...
                    if (existiertNam == 0)
                      printf("%s, %d, %d, %d, %d\n", namName[nam], namZ[nam], distanz[j], abstX, abstY) > namTmpFile;
                    else
                      existiertNam = 0;
...
function namBeschreibung(namTyp) {
  if (namTyp == "sn10")
    return "swissNAMES3D";
  else if (namTyp == "sn200")
    return "swissTLM-Regio Names";
  else if (namTyp == "sn500")
    return "DKM500 Namen";
  else if (namTyp == "sn1000")
    return "DKM1000 Namen";
  else if (namTyp == "sn10000")
...
function usage() {
  printf("\n%s\n", rep(146, "*"));
  printf("  Usage: skyplot2pano.awk  <X> <Y> <Z> <Name> <DHM> <Aufloes-Azi> <Azi links> <Azi rechts> <Bildbreite> <Min-Dist> <Max-Dist> <Aufloes-Dist> <Nam>\n");
  printf("%s\n\n", rep(146, "*"));
  ...
  printf("Beispiel\n");
  printf("%s\n", rep(8, "-"));
  printf("  skyplot2pano.awk 610558 197236 635 Gantrischweg dhm25 0.1 200 250 400 0 40 1 sn200\n");
}
