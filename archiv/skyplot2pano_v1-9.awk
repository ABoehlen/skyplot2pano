...
function bildkooX(xP, yP, xE, yE, aziLi, aziRe, bildbr, radPr,    alpha) {
  if (xE > xP && yE > yP) # Nordost
    alpha = rad2gon(atan2(xE - xP, yE - yP));
  else if (xE > xP && yE < yP) # Suedost
    alpha = rad2gon(atan2(xE - xP, yP - yE)) + 100;
  else if (xE < xP && yE < yP) # Suedwest
    alpha = rad2gon(atan2(xP - xE, yP - yE)) + 200;
  else if (xE < xP && yE > yP) # Nordwest
    alpha = rad2gon(atan2(xP - xE, yE - yP)) + 300;
  else if (xE == xP && yE > yP) # Nord
    alpha = 0;
  else if (xE > xP && yE == yP) # Ost
    alpha = 100;
  else if (xE == xP && yE < yP) # Sued
    alpha = 200;
  else if (xE < xP && yE == yP) # West
    alpha = 300;
}
